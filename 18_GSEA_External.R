#=========================Script Description=================================
# This script is used for GSEA analysis
#===========================Loading Packages=============================
...
option_list <- list(
  make_option(c("--input", "-i"),
              type = "character", default = "External/ALS_vs_ctrl/als_vs_ctrl.csv",
              help = "External/ALS_vs_ctrl/als_vs_ctrl.csv"
  ),make_option(c("--output", "-o"),
                type = "character", default = "18_GSEA_External",
...
result$signed_pval <- -log10(result$als_vs_ctrl_p.val) * sign(result$als_vs_ctrl_diff)
...
p <- ggplot(gsea_df, aes(x = signed_logp, y = Description_trunc, fill = NES > 0)) +
  ...
  scale_fill_manual(values = c("TRUE" = "firebrick3", "FALSE" = "steelblue3"),
                    labels = c("Down in ALS", "Up in ALS"),
                    name = "") +
  labs(
