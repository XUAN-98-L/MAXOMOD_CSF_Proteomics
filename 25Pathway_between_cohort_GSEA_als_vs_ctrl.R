...
Discovery = readRDS("Discovery/19_GSEA_allsamples/GSEA_result.rds")
Discovery = Discovery$BP@result

Validation = readRDS("Validation/19_GSEA_allsamples/GSEA_result.rds")
...
output_dir <- "25Pathway_between_cohort_GSEA_als_vs_ctrl"
...
cut_off = -log10(0.05)
main_title = "ALS vs CTRL (GSEA p.adjust)"
max.overlaps = 10
...
p = ggplot(data, aes(x, y, text = name)) +  # Include 'text' in aes for interactive tooltip
  ...
  geom_text_repel(data = filter(data, abs(y) >= cut_off | abs(x) >= cut_off), 
                  aes(label = name), 
                  size = 1.5, 
                  min.segment.length = 0, # **确保所有标签都有连接线**
                  direction = "both",    # 允许标签在四周调整位置
                  segment.color = "grey50",  # 连接线颜色
                  segment.size = 0.3,    # 连接线宽度
                  max.overlaps = Inf,    # 显示所有标签
                  box.padding = 0.3,     # 文字与点之间的距离
                  point.padding = 0.2) 
