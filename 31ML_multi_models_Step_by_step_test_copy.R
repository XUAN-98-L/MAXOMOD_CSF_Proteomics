...
# opt$output = "31_ML_multi_models_new"
#Rscript ~/Documents/Projects/MAXOMOD/Script/31ML_multi_models_Script.R --n_boot 1000
#===========================Loading Packages=============================
...
runML_with_plot <- function(discovery_df, validation_df,
  ...
  for (i in seq_len(BS_number)) {
    ...
    train_scaled <- scale_func(train_bs[, -ncol(train_bs),drop = FALSE])
    train_scaled$class <- train_bs$class
    
    # Validation
    #X_valid_scaled <- scale_func(validation_df[, -ncol(validation_df)])
    X_valid_scaled <- scale_func(validation_df[, -ncol(validation_df),drop = FALSE])
    ...
    # ridge regression
    if (dim(train_scaled)[2] == 2){
    # Caret: glmnet warning - x should be a matrix with 2 or more columns
    ridgeGrid <- expand.grid(alpha = 0,
                         lambda = 10^seq(-3, 1, length = 100)) 
    train_scaled_addone <-  train_scaled
    train_scaled_addone$one = rep(0, nrow(train_scaled_addone))                   
    ridge_model <- train(class ~ ., data = train_scaled_addone, method = "glmnet",
                         metric = "ROC", trControl = train_control, tuneLength = 5, tuneGrid = ridgeGrid)

    # lasso regression
    lassoGrid <- expand.grid(alpha = 1,
                         lambda = 10^seq(-3, 1, length = 100)) 
    lasso_model <- train(class ~ ., data = train_scaled_addone, method = "glmnet",
    ...
    if (dim(train_scaled)[2] == 2){
    X_valid_scaled_addone <- X_valid_scaled
    X_valid_scaled_addone$one = rep(0, nrow(X_valid_scaled_addone))
    p_ridge <- get_probs(ridge_model, X_valid_scaled_addone, y_valid)
    p_lasso <- get_probs(lasso_model, X_valid_scaled_addone, y_valid)
    }else{
    ...
    X_valid_scaled_addone <- X_valid_scaled
    X_valid_scaled_addone$one = rep(0, nrow(X_valid_scaled_addone))
    c_ridge <- get_class(ridge_model, X_valid_scaled_addone)
    c_lasso <- get_class(lasso_model, X_valid_scaled_addone)
    }else{
...
make_option(c("--output", "-o"),
                type = "character", default = "31_ML_multi_models_Step_by_step_copy",
  ...
  ),make_option(c("--topn", "-t"),
                type = "integer", default = 5,
                help = "Number of top features to select based on frequency."
...
if (is.null(opt$topn)) {
  print("NO TOPN VALUE SUPPLIED, default topn value will be used!")
  topn <- 10
} else {
  topn <- opt$topn
}
#============================================================================
Discovery = readRDS("Discovery/2_Missing_Inspection_als/norm_imp_MinProb.rds")
Validation = readRDS("Validation/2_Missing_Inspection_als/norm_imp_MinProb.rds")
...
df = read.csv("Discovery/8_Differential_expression_analysis_subclusters/Differential_expression_analysis_for_k2.csv")
DEP = df$name[df$fdr < 0.05]
DEP = intersect(DEP,inter)
inter = DEP

Discovery_als_ctrl = readRDS("Discovery/2_Missing_Inspection/norm_imp_MinProb.rds")
Validation_als_ctrl = readRDS("Validation/2_Missing_Inspection/norm_imp_MinProb.rds")
...
cluster_result = read.csv("Discovery/5_Clustering_als/cluster_assignments_2.csv")
...
# sort proteins based on their frequency, select top n (this number is adjustable)
selected_final <- names(feature_freq)[order(feature_freq, decreasing = TRUE)]
selected_final <- selected_final[1:topn]
...
validation_cluster_result = read.csv("Validation/5_Clustering_als/cluster_assignments_2.csv")
...
feature_plot <- ggplot(feature_freq_df, aes(x = reorder(Protein, -Frequency), y = Frequency)) +
  ...
  geom_hline(yintercept = feature_freq[selected_final[topn]], linetype = "dashed", color = "red", size = 1) +  # Add dashed red line at the frequency of the topn-th feature
  coord_flip() +
...
# ===========================Step-by-step Protein Addition Analysis=============================
message("Starting step-by-step protein addition analysis...")

# Function to run ML models with a specific set of proteins
run_ml_with_proteins <- function(discovery_data, validation_data, protein_set, n_boot = 10) {
  # Check which proteins are actually available in the data
  available_proteins <- intersect(protein_set, colnames(discovery_data))
  missing_proteins <- setdiff(protein_set, colnames(discovery_data))
  
  if (length(missing_proteins) > 0) {
    message("Warning: The following proteins are not found in the data: ", paste(missing_proteins, collapse = ", "))
  }
  
  if (length(available_proteins) == 0) {
    stop("None of the specified proteins are found in the data!")
  }
  
  # Subset data to include only the available proteins
  discovery_subset <- discovery_data[, c(available_proteins, "class")]
  validation_subset <- validation_data[, c(available_proteins, "class")]
  
  # Run ML models
  results <- runML_with_plot(discovery_subset, validation_subset,
                            BS_number = n_boot, 
                            cv = 10, 
                            plot_mean_roc = FALSE, 
                            output_dir = NULL)
  
  # Extract mean AUC for each model
  auc_summary <- data.frame(
    Model = names(results$auc_list),
    Mean_AUC = sapply(results$auc_list, function(x) mean(x, na.rm = TRUE)),
    SD_AUC = sapply(results$auc_list, function(x) sd(x, na.rm = TRUE)),
    stringsAsFactors = FALSE
  )
...
# Get all selected proteins ordered by frequency
all_proteins <- names(feature_freq)[order(feature_freq, decreasing = TRUE)]

# Get the actual protein column names from the discovery data
available_protein_columns <- colnames(discovery_selected)[colnames(discovery_selected) != "class"]

# Debug: Check what proteins are available in the data
message("Available proteins in discovery data: ", length(available_protein_columns))
message("First 10 proteins in discovery data: ", paste(head(available_protein_columns, 10), collapse = ", "))
message("First 10 proteins from feature selection: ", paste(head(all_proteins, 10), collapse = ", "))

# Filter all_proteins to only include those that are actually in the data
all_proteins <- intersect(all_proteins, available_protein_columns)
message("Proteins available for step-by-step analysis: ", length(all_proteins))

# Start with top 1 protein and incrementally add one protein at a time up to 10 proteins
step_results <- list()
step_summary <- data.frame()

# Ensure we have at least 1 protein to start with
if (length(all_proteins) < 1) {
  stop("Not enough proteins available for step-by-step analysis. Need at least 1 protein.")
}

for (i in 1:min(length(all_proteins), 10)) {  # Test from 1 to 10 proteins
  current_proteins <- all_proteins[1:i]
  message(paste("Testing with", i, "proteins:", paste(current_proteins, collapse = ", ")))
  
  # Run ML models with current protein set
  current_results <- run_ml_with_proteins(discovery_selected, validation_selected, 
                                        current_proteins, n_boot = n_boot)
  
  # Store results
  step_results[[paste0("step_", i)]] <- current_results
  
  # Add to summary
  current_summary <- current_results$auc_summary
  current_summary$Step <- i
  current_summary$Proteins <- paste(current_proteins, collapse = ";")
  current_summary$Added_Protein <- ifelse(i > 1, all_proteins[i], "Initial")
  
  step_summary <- rbind(step_summary, current_summary)
}

# Create performance comparison plot
performance_plot <- ggplot(step_summary, aes(x = Step, y = Mean_AUC, color = Model)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = Mean_AUC - SD_AUC, ymax = Mean_AUC + SD_AUC), 
                width = 0.1, alpha = 0.7) +
  #facet_wrap(~Model, scales = "free_y") +
  labs(title = "Model Performance with Incremental Protein Addition",
       subtitle = "Starting from top 1 protein, adding one protein at a time up to 10 proteins",
       x = "Number of Proteins", 
       y = "Mean AUC ± SD") +
  scale_color_brewer(palette = "Set2") +  # Use same color scheme as runML_with_plot
  theme_classic(base_size = 12) +
  #theme(legend.position = "none") +
  scale_x_continuous(breaks = 1:max(step_summary$Step))

saveRDS(step_summary, file.path(output_dir, "step_by_step_summary.rds"))
saveRDS(performance_plot, file.path(output_dir, "step_by_step_performance.rds"))
# Save performance plot
ggsave(file.path(output_dir, "step_by_step_performance.pdf"), 
       plot = performance_plot, width = 12, height = 8, dpi = 300)
ggsave(file.path(output_dir, "step_by_step_performance.png"), 
       plot = performance_plot, width = 12, height = 8, dpi = 300)

# # Create a summary table showing the best performing model at each step
# best_per_step <- step_summary %>%
#   group_by(Step) %>%
#   slice_max(Mean_AUC, n = 1) %>%
#   select(Step, Model, Mean_AUC, SD_AUC, Added_Protein) %>%
#   arrange(Step)

# Save summary results
write.csv(step_summary, file.path(output_dir, "step_by_step_summary.csv"), row.names = FALSE)
#write.csv(best_per_step, file.path(output_dir, "best_model_per_step.csv"), row.names = FALSE)


# Save the step results for further analysis
saveRDS(step_results, file.path(output_dir, "step_by_step_results.rds"))

# ===========================Additional Analysis: Protein Contribution=============================
# Calculate the performance improvement contributed by each protein
performance_improvement <- data.frame()

for (model in unique(step_summary$Model)) {
  model_data <- step_summary[step_summary$Model == model, ]
  model_data <- model_data[order(model_data$Step), ]
  
  for (i in 2:nrow(model_data)) {
    improvement <- model_data$Mean_AUC[i] - model_data$Mean_AUC[i-1]
    protein_added <- model_data$Added_Protein[i]
    
    performance_improvement <- rbind(performance_improvement, data.frame(
      Model = model,
      Step = model_data$Step[i],
      Added_Protein = protein_added,
      AUC_Improvement = improvement,
      Previous_AUC = model_data$Mean_AUC[i-1],
      New_AUC = model_data$Mean_AUC[i]
    ))
  }
}

# Create protein contribution plot
contribution_plot <- ggplot(performance_improvement, aes(x = Step, y = AUC_Improvement, fill = Model)) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  facet_wrap(~Model, scales = "free_y") +
  labs(title = "Performance Improvement by Each Added Protein",
       subtitle = "AUC improvement contributed by each protein addition",
       x = "Step (Number of Proteins)", 
       y = "AUC Improvement") +
  scale_fill_brewer(palette = "Set2") +  # Use same color scheme as runML_with_plot
  theme_classic(base_size = 12) +
  theme(legend.position = "none") +
  scale_x_continuous(breaks = 2:max(performance_improvement$Step))

# Save contribution plot
ggsave(file.path(output_dir, "protein_contribution_analysis.pdf"), 
       plot = contribution_plot, width = 12, height = 8, dpi = 300)
ggsave(file.path(output_dir, "protein_contribution_analysis.png"), 
       plot = contribution_plot, width = 12, height = 8, dpi = 300)

# Create a summary of most/least contributing proteins
protein_contribution_summary <- performance_improvement %>%
  group_by(Added_Protein) %>%
  summarise(
    Mean_Improvement = mean(AUC_Improvement, na.rm = TRUE),
    Max_Improvement = max(AUC_Improvement, na.rm = TRUE),
    Min_Improvement = min(AUC_Improvement, na.rm = TRUE),
    Models_Improved = sum(AUC_Improvement > 0, na.rm = TRUE),
    Total_Models = n(),
    .groups = "drop"
  ) %>%
  arrange(desc(Mean_Improvement))

write.csv(protein_contribution_summary, 
          file.path(output_dir, "protein_contribution_summary.csv"), 
          row.names = FALSE)

write.csv(performance_improvement, 
          file.path(output_dir, "detailed_protein_contributions.csv"), 
          row.names = FALSE)

# Print top contributing proteins
message("Top 5 proteins contributing to performance improvement:")
print(head(protein_contribution_summary, 5))

message("Bottom 5 proteins (least contribution or negative impact):")
print(tail(protein_contribution_summary, 5))

message("Step-by-step protein addition analysis completed successfully!")